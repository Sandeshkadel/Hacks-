// Render contact + (optionally) club info on contact.html. Containers: #contact-cards and #club-info
(function(){
  ...
  async function renderContacts(){
    const box = $('#contact-cards') || $('#contact-grid');
    if (!box) return;
    ...
    if (!items.length){
      box.innerHTML = '<div class="card soft"><p class="muted">No contact details yet.</p></div>';
      return;
    }
  ...
  async function renderInfo(){
    ...
    if (!keys.length){
      box.innerHTML = '<div class="card soft"><p class="muted">No information yet.</p></div>';
      return;
    }
