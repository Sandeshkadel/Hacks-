// Replace the empty strings with your actual Firebase Web App config values
window.AppConfig = {
  firebase: {
    apiKey: "AIzaSyD5Mfvjir8lhmRoRZxWjXIv54JXiYYyL0A",
    authDomain: "techcrafters-2919c.firebaseapp.com",
    projectId: "techcrafters-2919c",
    storageBucket: "techcrafters-2919c.firebasestorage.app",
    messagingSenderId: "1039652089214",
    appId: "1:1039652089214:web:dc55c2fd07fee95c5e972f"
  },
  // optional: EmailJS if you want real emails instead of mock outbox
  email: {
    emailjs: {
      serviceId: "",    // e.g. "service_abc123"
      templateId: "",   // e.g. "template_xyz789"
      publicKey: ""     // e.g. "KLMNO_PQR"
    }
  }
};
